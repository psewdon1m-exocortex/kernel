      ...
      <div ref={workspaceRef} className="on-workspace">
        ...
        >
          ...
          <div className="on-world" style={{ transform: worldTransform }}>
            ...
            }} onParameterCommit={updateNodeParameter} onFileDrop={(nodeId, file) => void importFileToNode(nodeId, file)} onOpenAsset={(asset) => void openAssetFromNode(asset)} onDownloadAsset={downloadAssetFromNode} />)}
...
function ContainerView({ container, project, definitions, session, selected, dragging, dropTarget, externalDropIndex, offset, previewRect, readOnly, onPointerDown, onSelect, onActivate, onContextMenu, onResize, onRename, onStartConnection, onFinishConnection, onReorder, onSelectNode, onDuplicateNodeForDrag, onNodeDragState, onParameterCommit, onFileDrop, onOpenAsset, onDownloadAsset }: {
  ...
  onOpenAsset(asset: OpenNodeProject["assets"][number]): void; onDownloadAsset(asset: OpenNodeProject["assets"][number]): void;
}) {
...
function ContainedNodePreview({ node, definition, asset, status, readOnly, onSelect, onContextMenu, onParameterCommit, onFileDrop, onOpenAsset, onDownloadAsset, onDragStart, onDragEnd }: {
  node: NodeInstance; definition?: NodeDefinition; asset?: OpenNodeProject["assets"][number]; status: string; readOnly: boolean; onSelect(): void; onParameterCommit(nodeId: string, parameterId: string, value: unknown): void;
  onContextMenu(event: ReactMouseEvent): void; onFileDrop(file: File): void; onOpenAsset(asset: OpenNodeProject["assets"][number]): void; onDownloadAsset(asset: OpenNodeProject["assets"][number]): void; onDragStart(event: DragEvent<HTMLDivElement>): void; onDragEnd(): void;
}) {
  const inputs = node.ports.filter((port) => port.direction === "input");
  const outputs = node.ports.filter((port) => port.direction === "output");
  const browserDocument = isBrowserDocumentDefinition(definition);
  return <div className={`on-contained-node ${isTextCanvasNode(node) ? "is-text-node" : ""} ${browserDocument ? "is-browser-document" : ""} ${node.bypassed ? "is-bypassed" : ""}`} style={{ "--element-color": node.color ?? definition?.defaultColor ?? "#667085" } as CSSProperties} data-node-id={node.id} data-node-type-id={node.nodeTypeId} draggable={!readOnly} onDragStart={onDragStart} onDragEnd={onDragEnd} onDragOver={(event) => { if ((node.nodeTypeId === "open-node.import.universal" || browserDocument) && event.dataTransfer.types.includes("Files")) event.preventDefault(); }} onDrop={(event) => { const file = event.dataTransfer.files[0]; if ((node.nodeTypeId === "open-node.import.universal" || browserDocument) && file) { event.preventDefault(); event.stopPropagation(); onFileDrop(file); } }} onClick={(event) => { event.stopPropagation(); onSelect(); }} onContextMenu={onContextMenu}>
    ...
    <div className="on-contained-node-body">{browserDocument
      ? <BrowserDocumentPanel asset={asset} readOnly={readOnly} onFile={onFileDrop} onOpen={onOpenAsset} onDownload={onDownloadAsset} />
      : <><span className="is-inputs">{inputs.slice(0, 2).map((port) => <i key={port.id}>{port.label}</i>)}</span><span className="is-summary on-contained-node-parameters" draggable={false} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} onDragStart={(event) => { event.preventDefault(); event.stopPropagation(); }}><ParameterSummary node={node} definition={definition} readOnly={readOnly} onCommit={onParameterCommit} /></span><span className="is-outputs">{outputs.slice(0, 2).map((port) => <i key={port.id}>{port.label}</i>)}</span></>}
    </div>
  </div>;
}
